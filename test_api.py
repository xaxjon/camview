...
s, j = api("camera-test.php", "POST", {"source": "rtsp://127.0.0.1:18554/hev"}, csrf)
check("probe reports HEVC with transcode hint", s == 200 and j.get("ok")
      and "HEVC" in j.get("video", "") and "transcode video" in (j.get("hint") or ""), j)

print("== transcode_video flag ==")
s, j = api("cameras.php", "PUT", {"original": "testcam", **cam, "transcode_video": True}, csrf)
check("set transcode_video", s == 200, j)
sj = json.loads(pathlib.Path("/tmp/camview-test/streams.json").read_text())
entry = next(e for e in sj if isinstance(e, dict) and e.get("name") == "testcam")
check("transcode_video persisted", entry.get("transcode_video") is True, entry)
with urllib.request.urlopen(MTX + "/v3/config/paths/get/testcam") as r:
    conf = json.loads(r.read())
check("public path re-encodes video", " h264" in conf.get("runOnDemand", ""), conf)
s, j = api("cameras.php", "PUT", {"original": "testcam", **cam, "transcode_video": False}, csrf)
with urllib.request.urlopen(MTX + "/v3/config/paths/get/testcam") as r:
    conf = json.loads(r.read())
check("flag off restores stream copy", " h264" not in conf.get("runOnDemand", ""), conf)
s, j = api("camera-test.php", "POST", {"source": "rtsp://127.0.0.1:18554/test"}, csrf)
